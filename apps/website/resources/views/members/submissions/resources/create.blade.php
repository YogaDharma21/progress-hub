...
@section('title', 'Progress Hub — Submit Resource Baru')

@section('content')
<div class="space-y-8 max-w-4xl mx-auto">
    <form action="{{ route('members.submissions.resources.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-800 pb-6">
            <div>
                ...
                <h1 class="text-2xl font-bold text-zinc-100 tracking-tight">Submit Resource Baru</h1>
                <p class="mt-1 text-xs text-zinc-400">Isi detail modul, artikel, atau tutorial. Akan dikirim ke admin untuk review.</p>
            </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <label for="cover" class="relative flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-zinc-800 hover:border-zinc-700 bg-zinc-950 rounded-xl cursor-pointer transition overflow-hidden group">
                        ...
                        <div id="cover-preview-container" class="hidden absolute inset-0 w-full h-full">
                            ...
                            <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition flex items-center justify-center text-xs font-semibold text-white">Klik untuk ganti cover</div>
                        </div>
