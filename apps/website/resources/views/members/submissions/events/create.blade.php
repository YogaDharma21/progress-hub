...
<div class="space-y-8 max-w-4xl mx-auto">
    <form action="{{ route('members.submissions.events.store') }}" method="POST" class="space-y-6">
        @csrf
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-800 pb-6">
            <div>
                ...
                <h1 class="text-2xl font-bold text-zinc-100 tracking-tight">Submit Event Baru</h1>
                <p class="mt-1 text-xs text-zinc-400">Isi detail program kerja atau kegiatan. Akan dikirim ke admin untuk review.</p>
            </div>
        ...
        <div class="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-sm">
            ...
            <div class="mt-5 space-y-4">
                <div class="space-y-1.5">
                    ...
                    <input id="title" name="title" type="text" value="{{ old('title') }}" required placeholder="Contoh: Kelas React Dasar"
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea id="description" name="description" rows="4" placeholder="Jelaskan gambaran umum kegiatan ini..."
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="space-y-1.5">
                        ...
                        <select id="status" name="status" class="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-sm text-zinc-100 focus:outline-none focus:ring-2 focus:ring-zinc-400 focus:border-zinc-700 transition cursor-pointer">
                            ...
                            <option value="Registration" {{ old('status') == 'Registration' ? 'selected' : '' }}>Registration</option>
                        </select>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="space-y-1.5">
                        ...
                        <input id="sessions_count" name="sessions_count" type="number" min="0" value="{{ old('sessions_count') }}" placeholder="Contoh: 8"
                            class="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-sm text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-zinc-400 focus:border-zinc-700 transition" />
                    </div>

                    <div class="space-y-1.5">
                        <label for="target_capacity" class="block text-xs font-medium text-zinc-300">Target / Kapasitas Peserta</label>
                        <input id="target_capacity" name="target_capacity" type="number" min="0" value="{{ old('target_capacity') }}" placeholder="Contoh: 42"
                            class="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-sm text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-zinc-400 focus:border-zinc-700 transition" />
                    </div>
                </div>
            </div>
            ...
            <div id="topics-container" class="mt-5 grid grid-cols-1 md:grid-cols-2 gap-5">
                <div class="topic-item bg-zinc-950 border border-zinc-800 rounded-xl p-5 space-y-4 relative group hover:border-zinc-700 transition shadow-sm">
                    <div class="flex items-center justify-between pb-1">
                        <span class="topic-label inline-flex items-center px-3 py-1 rounded-md text-xs font-semibold bg-zinc-800 text-zinc-200 border border-zinc-700">Topik 01</span>
