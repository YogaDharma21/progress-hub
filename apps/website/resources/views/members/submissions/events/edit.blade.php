...
<div class="space-y-8 max-w-4xl mx-auto">
    <form action="{{ route('members.submissions.events.update', $event) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-800 pb-6">
            <div>
                ...
                <p class="mt-1 text-xs text-zinc-400">Perbarui detail program kerja atau kegiatan.</p>
            </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="space-y-1.5">
                        ...
                        <select id="status" name="status" class="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-sm text-zinc-100 focus:outline-none focus:ring-2 focus:ring-zinc-400 focus:border-zinc-700 transition cursor-pointer">
                            ...
                            <option value="Registration" {{ old('status', $event->status) == 'Registration' ? 'selected' : '' }}>Registration</option>
                        </select>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select id="type" name="type" class="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-sm text-zinc-100 focus:outline-none focus:ring-2 focus:ring-zinc-400 focus:border-zinc-700 transition cursor-pointer">
                            ...
                            <option value="Kelas" {{ old('type', $event->type) == 'Kelas' || old('type', $event->type) == 'class' ? 'selected' : '' }}>Kelas</option>
                            <option value="Hackathon" {{ old('type', $event->type) == 'Hackathon' || old('type', $event->type) == 'hackathon' ? 'selected' : '' }}>Hackathon</option>
                            <option value="Sharing" {{ old('type', $event->type) == 'Sharing' || old('type', $event->type) == 'sharing' ? 'selected' : '' }}>Sharing</option>
                        </select>
                    </div>
                </div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-zinc-800">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-zinc-400 mt-0.5">Kelola daftar materi yang akan dipelajari peserta</p>
                    </div>
                </div>
        ...
        addBtn.addEventListener('click', function () {
            ...
            newItem.innerHTML = `
                ...
                <div class="space-y-3">
                    <div class="space-y-1.5">
                        <label class="block text-[11px] font-medium text-zinc-400">Judul Topik</label>
                        <input type="text" name="topics[${index}][title]" placeholder="Contoh: Judul Topik Baru"
                            class="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-3.5 py-2.5 text-xs text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-zinc-400 transition font-medium" />
                    </div>
                    <div class="space-y-1.5">
                        <label class="block text-[11px] font-medium text-zinc-400">Penjelasan Ringkas</label>
